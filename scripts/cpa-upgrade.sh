...
log "remote binary sha=$REMOTE_SHA"
[ "$LOCAL_BIN_SHA" = "$REMOTE_SHA" ] || { log "SHA mismatch!"; exit 1; }
$SSH "chmod +x /home/azureuser/CLIProxyAPIPlus-new/cpa-new-server.new"

log "=== 4. probe new binary on VPS:$PROBE_PORT ==="
...
pkill -f "cpa-new-server.new.*probe" 2>/dev/null || true
sleep 1
...
nohup ./cpa-new-server.new -config cpa-new-config-probe.yaml >"\$PROBE_LOG" 2>&1 &
PROBE_PID=\$!
