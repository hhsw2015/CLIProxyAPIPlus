...
def generate_bedrock(nd):
    ...
    # 3. ClaudeAws (config_us_k8s.json - legacy single AK/SK)
    ...
    for (ak, region), info in sorted(groups.items()):
        lines = [
            ...
            f"  aws-region: {region}",
            f"  priority: 10",
            f"  excluded-models: [claude-opus-4-7, claude-opus-4.7]",
            f"  models:"
        ]
...
def generate_azure(nd):
    ...
    for sect, prio, prefix in sections:
        ...
        # Merge US (full-config.json) + CN (keys-azure.json)
...
def main():
    ...
    # p10 Bedrock
    sections['claude-api-key'].extend(generate_bedrock(nd))
    ...
    sections['openai-compatibility'].extend(generate_vertex_xmind(nd))
    # p10 Gemini API keys
    sections['gemini-api-key'].extend(generate_gemini_keys(nd))
    # p10/p8 Azure
    sections['openai-compatibility'].extend(generate_azure(nd))
    ...
    sections['claude-api-key'].extend(generate_maas(nd))
    ...
    # p7 Media Direct
    sections['openai-compatibility'].extend(generate_media_direct(nd))
    # p6 MaaS tier (Deepseek/Grok/DashScope/ApiCoco)
    sections['openai-compatibility'].extend(generate_maas_tier(nd))
    # p5 Legacy (Silicon/DeepInfra/Sophnet/Cloudsway)
    sections['openai-compatibility'].extend(generate_legacy(nd))
    # p3 Shubiaobiao
    sections['openai-compatibility'].extend(generate_shubiaobiao(nd))
    # p2 Pools (Cookie + Singularity, ultra + plus)
    sections['openai-compatibility'].extend(generate_pools())
    # p1 gpt-proxy media fallback (21 routes)
    sections['openai-compatibility'].extend(generate_gpt_proxy_media(nd))
