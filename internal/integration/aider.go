...
func (m Manager) aiderStatus() (Status, error) {
	...
	if baseURL == m.advertisedBaseURL() && strings.HasPrefix(strings.TrimSpace(model), "openai/") {
		status.State = StateConfigured
	}
	return status, nil
}

func (m Manager) previewAider() (Preview, error) {
	targetPath, err := m.aiderTargetPath()
	if err != nil {
		return Preview{}, err
	}
...
func (m Manager) applyAider() (Result, error) {
	status, err := m.aiderStatus()
	if err != nil {
		return Result{}, err
	}
	...
	return Result{
		Product: ProductAider,
		Status:  status,
		Message: "Aider configured to use CPA",
	}, nil
}
