...
func (m Manager) geminiStatus() (Status, error) {
	...
	if geminiEnvValue(string(raw), geminiAPIBaseKey) == m.advertisedBaseURL() {
		status.State = StateConfigured
	}
	return status, nil
}

func (m Manager) previewGemini() (Preview, error) {
	targetPath, err := m.geminiTargetPath()
	if err != nil {
		return Preview{}, err
	}
	...
	planned := upsertEnvLine(current, geminiAPIBaseKey, m.advertisedBaseURL())
	return Preview{
		Product:        ProductGeminiCLI,
...
func (m Manager) applyGemini() (Result, error) {
	status, err := m.geminiStatus()
	if err != nil {
		return Result{}, err
	}
	...
	return Result{
		Product: ProductGeminiCLI,
		Status:  status,
		Message: "Gemini CLI configured to use CPA",
	}, nil
}
