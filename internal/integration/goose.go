...
func (m Manager) gooseTargetPath() (string, error) {
	...
	return filepath.Join(home, ".config", "goose", "custom_providers", "cpa.json"), nil
}

func (m Manager) advertisedGooseBaseURL() string {
	return m.advertisedBaseURL() + "/v1/chat/completions"
}

func (m Manager) gooseStatus() (Status, error) {
	...
	status := Status{
		...
		Warning:         "Goose may still require you to select the CPA provider or model inside Goose, and environment overrides can still change behavior.",
	}
	...
	if baseURL == m.advertisedGooseBaseURL() && engine == "openai" {
		status.State = StateConfigured
	}
	return status, nil
}

func (m Manager) previewGoose() (Preview, error) {
	targetPath, err := m.gooseTargetPath()
	if err != nil {
		return Preview{}, err
	}
	...
	body := map[string]any{
		...
		"engine":       "openai",
		"display_name": "CPA",
		"description":  "CPA managed OpenAI-compatible provider",
		"base_url":     m.advertisedGooseBaseURL(),
		"models": []any{
			map[string]any{
...
func (m Manager) applyGoose() (Result, error) {
	status, err := m.gooseStatus()
	if err != nil {
		return Result{}, err
	}
	...
	return Result{
		Product: ProductGoose,
		Status:  status,
		Message: "Goose custom provider configured for CPA",
	}, nil
}
