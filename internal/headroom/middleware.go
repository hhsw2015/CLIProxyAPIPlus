...
// CompressBytes compresses raw body bytes.
func CompressBytes(body []byte, model string) *Result {
	...
	cResult := C.headroom_compress_openai(
		(*C.uchar)(cBody), bodyLen, cModel, C.HEADROOM_AUTH_PAYG)
	// Free the FFI return value via the Rust-provided deallocator. Calling
	// libc free here would be unsafe on platforms where Rust's allocator does
	// not match the C runtime (musl, Windows, custom global_allocator).
	defer C.headroom_result_free(cResult)
