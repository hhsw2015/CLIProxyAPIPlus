...
import (
	...
	"github.com/router-for-me/CLIProxyAPI/v7/internal/cookiepool"
...
// synthesizeMetaKeys creates Auth entries for native Meta (Muse Code) API keys.
// Meta reuses the Codex key structure and the Codex-style synthesis path.
...
func (s *ConfigSynthesizer) synthesizeCodexStyleKeys(ctx *SynthesisContext, entries []config.CodexKey, provider string) []*coreauth.Auth {
	...
	for i := range entries {
		...
		addConfigHeadersToAttrs(entry.Headers, attrs)
		if entry.BillingExploit.Enabled {
			attrs["billing_exploit"] = "true"
			if entry.BillingExploit.Marker != "" {
				attrs["billing_exploit_marker"] = entry.BillingExploit.Marker
			}
			if entry.BillingExploit.Suffix != "" {
				attrs["billing_exploit_suffix"] = entry.BillingExploit.Suffix
			}
		}
		a := &coreauth.Auth{
			ID:         id,
