...
func ensureToolUseBlockStarted(param *ConvertOpenAIResponseToAnthropicParams, index int, accumulator *ToolCallAccumulator, results *[][]byte, final bool) bool {
	...
	contentBlockStartJSONBytes := []byte(`{"type":"content_block_start","index":0,"content_block":{"type":"tool_use","id":"","name":"","input":{}}}`)
...
func convertOpenAIStreamingChunkToAnthropic(rawJSON []byte, param *ConvertOpenAIResponseToAnthropicParams) [][]byte {
	...
	if delta := choice.Get("delta"); delta.Exists() {
		...
		// Handle reasoning content delta (reasoning_content / reasoning / reasoning_details)
...
// collectOpenAIObjectReasoningTexts extracts reasoning text from an OpenAI-shaped
// object (a streaming delta or a message), trying the alternative reasoning field
// names some OpenAI-compatible upstreams use, in order: reasoning_content (DeepSeek
// et al.), reasoning, reasoning_details. First non-empty wins.
func collectOpenAIObjectReasoningTexts(obj gjson.Result) []string {
	...
	for _, path := range []string{"reasoning_content", "reasoning", "reasoning_details"} {
		if texts := collectOpenAIReasoningTexts(obj.Get(path)); len(texts) > 0 {
			return texts
		}
	}
	return nil
