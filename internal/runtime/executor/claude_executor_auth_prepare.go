...
// Ported from upstream's claude_executor_auth.go split file so the fork's
// monolithic Claude executor can still lazily populate a Claude OAuth
// credential's account UUID and canonicalize the device-ID pool before its
...
// ShouldPrepareRequestAuth reports whether the executor needs to derive
// credential identity before dispatching a request. It returns true for Claude
// OAuth credentials that still lack a canonical device pool or account UUID.
...
// PrepareRequestAuth backfills credential identity (device pool + account
// UUID) for a Claude OAuth credential. Callers should invoke it before
// executing a request when ShouldPrepareRequestAuth returns true.
