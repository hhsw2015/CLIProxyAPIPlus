...
func TestServiceRunBootstrapsSnapshotAuthsWithPoolModeEnabled(t *testing.T) {
	authID := "skywork-pool-bootstrap"
	reg := GlobalModelRegistry()
	reg.UnregisterClient(authID)
	t.Cleanup(func() {
		reg.UnregisterClient(authID)
	...
	service, err := NewBuilder().
		...
		WithWatcherFactory(func(configPath, authDir string, reload func(*config.Config)) (*WatcherWrapper, error) {
			return &WatcherWrapper{
				...
				snapshotRootFileAuths: func() []*coreauth.Auth { return nil },
